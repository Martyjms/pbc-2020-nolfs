

<footer id="pre-load">
  <div class="container-fluid padding">
    <div class="row text-center">
      <div class="col-md-3">
      <img  src="{{asset('public\images\PBC_Logo2022-02.png')}}" alt="Plesantview Bible Camp Logo">

      </div>

      <div class="col-md-3">
        <hr class="light">
        <div class="container-fluid padding">
            <h5>Contact</h5>
        <hr class="light">
      <p>(306) 825 - 2461 (FBC Office)</p>
      <p>(306) 825 - 3415 (Camp)</p>
      <p>info@pleasantviewcamp.org</p>
      <p>3915 47 Ave, Lloydminster SK<br>S9V 2C7</p>
          </div>

      </div>

      <div class="col-md-3">
        <hr class="light">
        <h5>Upcoming Events</h5>
        <hr class="light">
        <li>Sr High Youth Retreat -- August 27 - 29</li>

        <br>
        {{-- <p>Camp Cleanup  May 15 2020</p> --}}
        <br>
        <br>
        {{-- <p>Staff & LIT Applications Due April 1</p> --}}
      </div>
      <div class="col-md-3">
        <hr class="light">
        <h5>Quick Links</h5>
        <hr class="light">
        <li><a href="register">Registration</a></li>
        <li><a href="about">About Pleasantview</a></li>
        <li><a href="director">Camp Director</a></li>
        <li><a href="staff">Staff</a></li>
        <li><a href="http://www.fbclloyd.ca/">FBC Lloyd</a></li>

      </div>
      <div class="col-12">
        <hr class="light-100">
        <h5>&copy;2021 pleasantviewcamp.org</h5>
      </div>
  </div>
  </div>
</footer>





{{-- <footer class="container py-5">
    <div class="row">
      <div class="col-12 col-md">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="d-block mb-2" role="img" viewBox="0 0 24 24" focusable="false"><title>Product</title><circle cx="12" cy="12" r="10"/><path d="M14.31 8l5.74 9.94M9.69 8h11.48M7.38 12l5.74-9.94M9.69 16L3.95 6.06M14.31 16H2.83m13.79-4l-5.74 9.94"/></svg>
        <small class="d-block mb-3 text-muted">&copy; 2019</small>
      </div>
      <div class="col-6 col-md">
        <h5>Features</h5>
        <ul class="list-unstyled text-small">
          <li><a class="text-muted" href="#">Cool stuff</a></li>
          <li><a class="text-muted" href="#">Random feature</a></li>
          <li><a class="text-muted" href="#">Team feature</a></li>
          <li><a class="text-muted" href="#">Stuff for developers</a></li>
          <li><a class="text-muted" href="#">Another one</a></li>
          <li><a class="text-muted" href="#">Last time</a></li>
        </ul>
      </div>
      <div class="col-6 col-md">
        <h5>Parents</h5>
        <ul class="list-unstyled text-small">
          <li><a class="text-muted" href="/parents">Resource</a></li>
          <li><a class="text-muted" href="#">Resource name</a></li>
          <li><a class="text-muted" href="#">Another resource</a></li>
          <li><a class="text-muted" href="#">Final resource</a></li>
        </ul>
      </div>
      <div class="col-6 col-md">
        <h5>Resources</h5>
        <ul class="list-unstyled text-small">
          <li><a class="text-muted" href="#">Business</a></li>
          <li><a class="text-muted" href="#">Education</a></li>
          <li><a class="text-muted" href="#">Government</a></li>
          <li><a class="text-muted" href="#">Gaming</a></li>
        </ul>
      </div>
      <div class="col-6 col-md">
        <h5>About</h5>
        <ul class="list-unstyled text-small">
          <li><a class="text-muted" href="#">Team</a></li>
          <li><a class="text-muted" href="#">Locations</a></li>
          <li><a class="text-muted" href="#">Privacy</a></li>
          <li><a class="text-muted" href="#">Terms</a></li>
        </ul>
      </div>
    </div>

<h5>2020 Summer Camps</h5>
        <hr class="light">
        <p>Sr High -- July 5-11</p>
        <p>Pre-teen 1 -- July 12-17</p>
        <p>Junior High 1 -- July 19-24 </p>
        <p>Pre-teen 2</p>
        <p>Primary</p>
        <p>Junior High 2</p>

  </footer> --}}
